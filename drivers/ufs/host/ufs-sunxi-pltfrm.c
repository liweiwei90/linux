...
#include "tc-dwc-sunxi.h"
#include "ufshci-sunxi.h"
#include "ufs-sunxi.h"

#define USE_UNUSED_CODE
#define SUNXI_UFS_DRIVER_VESION "0.0.28 2026.7.6"
...
#define SUNXI_UFS_SRAM_INIT_TIMEOUT_MS		100
...
static void sunxi_ufs_clear_cal_words(u32 *pll_rate_a, u32 *pll_rate_b,
...
static bool sunxi_ufs_cal_words_sane(u32 rval_l, u32 rval_h)
{
	u32 pll_a, pll_b, att0, ctle0, att1, ctle1;

	pll_a = (rval_h >> 16) & 0xff;
	pll_b = (rval_h >> 24) & 0xff;
	att0 = (rval_l >> 16) & 0xff;
	ctle0 = (rval_l >> 24) & 0xff;
	att1 = rval_h & 0xff;
	ctle1 = (rval_h >> 8) & 0xff;
	...
	if (pll_a && (pll_a < 0x14 || pll_a > 0x4b))
		return false;
	if (pll_b && (pll_b < 0x4b || pll_b > 0x73))
		return false;
	if (att0 && (att0 == 255 || !ctle0 || ctle0 == 255))
		return false;
	if (att1 && (att1 == 255 || !ctle1 || ctle1 == 255))
		return false;

	return true;
}
static int sunxi_ufs_read_sid_word(struct nvmem_device *sid, u32 offset, u32 *val)
{
	int len;

	len = nvmem_device_read(sid, offset, sizeof(*val), val);
	if (len < 0)
		return len;
	if (len != sizeof(*val))
		return -EINVAL;
										...
										u32 *att_lane1, u32 *ctle_lane1)
{
	...
	if (!priv || !priv->sid) {
		dev_dbg(hba->dev, "SID nvmem not bound, use auto cal\n");
		return 0;
	}

	dev_dbg(hba->dev, "Get ufs_cal_word_l from SID 0x%x\n",
		SUNXI_UFS_CAL_WORDS_EFUSE_ALIGN_LOW);
	ret = sunxi_ufs_read_sid_word(priv->sid,
				      SUNXI_UFS_CAL_WORDS_EFUSE_ALIGN_LOW,
				      &rval_l);
	if (ret) {
		dev_err(hba->dev, "Get ufs_cal_word_l failed (%d), use auto cal\n", ret);
		return 0;
	}

	dev_dbg(hba->dev, "Get ufs_cal_word_h from SID 0x%x\n",
		SUNXI_UFS_CAL_WORDS_EFUSE_ALIGN_HIGH);
	ret = sunxi_ufs_read_sid_word(priv->sid,
				      SUNXI_UFS_CAL_WORDS_EFUSE_ALIGN_HIGH,
				      &rval_h);
	if (ret) {
		dev_err(hba->dev, "Get ufs_cal_word_h failed (%d), use auto cal\n", ret);
		return 0;
	}
	...
	if (!sunxi_ufs_cal_words_sane(rval_l, rval_h)) {
		dev_warn(hba->dev, "ignore invalid efuse cal words, use auto cal\n");
		sunxi_ufs_clear_cal_words(pll_rate_a, pll_rate_b,
...
static int ufshcd_sunxi_dme_set_attrs(struct ufs_hba *hba,
...
static int sunxi_ufs_rmmi_config(struct ufs_hba *hba)
{
	...
	struct ufshcd_dme_attr_val rmmi_config[] = {
		 ...
		 * here bit 4 set to 1 too according to SNPS ip
		 */
	...
	/*
	 * Bad efuse cal values can cause UIC errors during link startup on
	 * SUN60IW2; force auto PLL cal unless valid words were read.
	 */
	if (of_property_read_bool(hba->dev->of_node, "force-auto-pll-cal"))
	...
	ret = sunxi_ufshcd_wait_for_register(hba, REG_UFS_CFG, MPHY_SRAM_INIT_DONE_BIT,
					     MPHY_SRAM_INIT_DONE_BIT, 10,
					     SUNXI_UFS_SRAM_INIT_TIMEOUT_MS);
	if (ret) {
		dev_err(hba->dev, "%s: wait sram init done timeout, REG_UFS_CFG=0x%x\n",
			__func__, ufshcd_readl(hba, REG_UFS_CFG));
		return ret;
	}
...
static int sunxi_ufs_sys_clk_init(struct ufs_hba *hba)
{
	...
	/* Only to avoid waring when disable clk and rst if no enable first Start */
...
static int ufs_ufs_parse_dt(struct device *dev, struct ufs_hba *hba)
{
	...
	priv->sid = devm_nvmem_device_get(dev, NULL);
	if (IS_ERR(priv->sid)) {
		ret = PTR_ERR(priv->sid);
		priv->sid = NULL;
		if (ret == -EPROBE_DEFER)
			return ret;
		dev_info(dev, "SID nvmem not available (%d), UFS will use auto cal\n",
			 ret);
	}
